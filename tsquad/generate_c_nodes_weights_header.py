    ...
    try:
        with open(fname, 'w') as f:
            ...
            for i in range(ts._N_tmax):
                ...
                for ki in range(k+1):
                    if ki == 0:
                        print("{:.15e}".format((ts._tmax_list[i] - ts._tmin) / 2 / N), end='', file=f)
                    else:
                        if ki == 1:
                            comment = "    //t_max {}".format(ts._tmax_list[i])
                        else:
                            comment = ''
                        print(",{}\n{} {:.15e}".format(comment, " " * (len(s)-1), (ts._tmax_list[i] - ts._tmin) / 2 / N), end='', file=f)
                    N *= 2
                print("}", end='', file=f)
            print("};\n", file=f)

            s = "static const double one_minus_g[{}][{}] = {{\n".format(ts._N_tmax, 2*N0*2**k+1)
            s_offs = 2
            for i in range(ts._N_tmax):
                if i == 0:
                    print(s, end='', file=f)
                ...
                for ki in range(k+1):
                    if ki == 0:
                        ...
                        print(",\n"+" "*s_offs+"  // {}. layer".format(ki), end='', file=f)
                        step = 2 ** (k - ki + 1)
                    ...
                    for i, ti in enumerate(t_slice):
                        print(newline, mp.nstr(_1mg(ti, mp.exp, mp.cosh, mp.sinh, mp.pi), 17, **kwargs), end='', file=f)
                        newline = ",{}\n".format(comment) + " "*(s_offs+1)
                        comment = ""
                print("}", end=(''), file=f)
            print("};\n", file=f)

            s = "static const double w[{}][{}] = {{".format(ts._N_tmax, 2*N0*2**k+1)
            s_offs = 2
            ...
            for i in range(ts._N_tmax):
                ...
                for ki in range(k+1):
                    ...
                    for i, ti in enumerate(t_slice):
                        print(newline, mp.nstr(_w(ti, mp.cosh, mp.sinh, mp.pi), 17, **kwargs), end='', file=f)
                        newline = ",{}\n".format(comment) + " "*(s_offs+1)
                        comment = ""
                print("}", end=(''), file=f)
            print("};\n", file=f)
            ...
            for i in range(ts._N_tmax):
                ...
                for ki in range(k+1):
                    ...
                    for i, ti in enumerate(t_slice):
                        ...
                        print(newline, mp.nstr(x/(1-x), 17, **kwargs), end='', file=f)
                        newline = ",{}\n".format(comment)+" "*(s_offs+1)
                        comment = ""
                print("}", end=(''), file=f)
            print("};\n", file=f)
